...
評価レポート本文を最終出力としてそのまま返す。ファイルには書かない（Bash のリダイレクトによる迂回も含めて書かない）。パイプラインが標準出力を `${RUN_DIR}/eval-<n>.md` として保存する。先頭行はバッククォート・太字などの装飾を付けず、素の `VERDICT: <PASS|NEEDS_REVISION|FAIL>` とする。続けて4軸ごとの PASS/FAIL と根拠（`path:line` 引用）・検証チェーン再実行結果・持ち越し指摘・次ループで直すべき項目を記す。
