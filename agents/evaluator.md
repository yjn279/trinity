...
評価は記事準拠の4軸で採点する。各軸の PASS は「プロダクトとしてユーザーへ本番提供できる品質基準を上回っている」ことを必要条件とする。すなわち、軸ごとに本番出荷に耐える水準を満たしているだけでなく、それを上回っているかを判断する。1軸でも本番品質に届かない、もしくはちょうど境界線にとどまる場合は PASS を出さない。具体的なチェック項目や閾値は計画の受け入れ基準に書かれているものを採用し、Evaluator は本ファイルで個別基準を列挙しない。
