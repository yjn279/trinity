...
| `NEEDS_REVISION` | 計画・要件自体が誤っている／再計画が必要なほど乖離。とくに**要件妥当性**が FAIL のとき（誤った要件を実装している）。道具パス（`/code-review --fix`・`/simplify`）の変更が `requirement.md` と食い違うときも、本物のリグレッションでない限りここに含める。 | Planner が再計画。要件そのものが疑わしい場合、Planner は `## 要確認の論点` でユーザーに差し戻す。道具の変更が理由の場合、Planner が `requirement.md` を更新する。 |
| `FAIL` | 不合格の指摘はあるが、計画は妥当で既存計画の範囲内で Generator が直せる。道具パスの変更が原因のときは、案件が実際に必要とする挙動が失われた本物のリグレッションに限る。 | Generator が修正。 |
