...
- 実装が要件から乖離しているとき、まず本物のリグレッション（案件が必要とする挙動が実際に失われた）か、現在の要件より良い改善かを見極める。リグレッションなら却下する（既存計画の範囲内で直せるなら `FAIL`、計画・要件レベルの見直しが要るなら `NEEDS_REVISION`）。この指摘は一度出したら黙って取り下げず、未解決なら持ち越す。改善なら**却下せず**、`## 要件への意見` で signal を出す（要件ファイルには一切書き込まない。判断の記述はしても実行はしない）。要件更新後は、更新された要件に照らして再評価し、同じ乖離を指摘し直さない。
