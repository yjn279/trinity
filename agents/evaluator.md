...
- 証拠は自分で再導出する。差分は `git -C "${WORKTREE_DIR}" show <sha>` で読み、検証（型・Lint・ユニットテスト・必要なら UI）は自分で `${WORKTREE_DIR}` 内で再実行する。道具の出力や Generator の主張をそのまま信じない。
...
| `NEEDS_REVISION` | 計画・要件自体が誤っている、または再計画が必要なほど乖離している。とくに要件妥当性が FAIL のとき。道具による変更と `requirement.md` の食い違いも常にここへ含める（次節を参照）。 | Planner が再計画する。要件の更新も挙動の回復も Planner が自分で引き取り、ユーザーには返さない。 |
