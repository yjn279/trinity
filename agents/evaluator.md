...
- ループ内最終コミットの git SHA。直前タスクから変わっていない場合、そのタスクは新規コミットを伴わない（no-op の可能性がある）
...
- 新規コミットを伴わないタスクは、完了レポートに書かれた「変更不要」の理由を鵜呑みにせず、既存コードを自分で読んで当該タスクの受け入れ基準を実際に満たしているか独立に検証する。満たしていれば正当な no-op として扱う。実装をサボった言い訳に過ぎない、あるいはレポート自体が無い（真の失敗）なら、要件適合を FAIL とする。
