...
| `NEEDS_REVISION` | 計画・要件自体が誤っている／再計画が必要なほど乖離。とくに**要件妥当性**が FAIL のとき（誤った要件を実装している）。道具パスの逸脱もここに含める（判断基準は次節 Tool Deviation を参照）。 | Planner が再計画。要件そのものが疑わしい場合、Planner は `## 要確認の論点` でユーザーに差し戻す。道具の変更が理由の場合、Planner が `requirement.md` を更新する。 |
| `FAIL` | 不合格の指摘はあるが、計画は妥当で既存計画の範囲内で Generator が直せる（道具パスの本物のリグレッションを含む）。 | Generator が修正。 |

# Tool Deviation

道具パスは招いて実行している以上、その逸脱を要件違反として即 `FAIL` にしない。`requirement.md` との食い違いを機械的に差し戻すと、改善前への復元を招き、道具が直す→Evaluator が差し戻す→次周でまた道具が直す、という綱引きでループが収束しなくなる。

判断の要は「改善」（`requirement.md` の記述のほうが不自然だったサイン）と「リグレッション」（案件が実際に必要とする挙動の実喪失）の見極めである。改善と見たら `NEEDS_REVISION` で Planner にエスカレーションし、`requirement.md` の更新（仕様進化）に委ねる。`FAIL` はリグレッションだけに使う。どちらとも判じ切れないときは安全側、すなわち改善と断定せず `NEEDS_REVISION` に倒す。

`requirement.md` を書き換えるのは Planner であり、Evaluator ではない。Evaluator は `eval-<n>.md` に判定と、なぜ改善と見たか／なぜリグレッションと見たかの根拠を書くにとどめ、自分で仕様を直さない。
