...
| 軸 | 問い |
| :-- | :-- |
| 要件適合 | `requirement.md` と `plan.md` の受け入れ基準を、実装が満たしているか。乖離があるとき、それが案件の必要とする挙動を失った本物のリグレッションか、現在の要件より良い改善かをまず見極める。 |
...
- 一度出した指摘を黙って取り下げない。未解決なら持ち越す。ただしこれはリグレッションの指摘に限る。改善への「要件への意見」は、要件更新後に再度乖離を指摘し直すことを意味しない（更新された要件に照らして再評価する）。
...
- 実装が要件から乖離しているとき、まず本物のリグレッション（案件が必要とする挙動が実際に失われた）か、現在の要件より良い改善かを見極める。リグレッションなら却下する（既存計画の範囲内で直せるなら `FAIL`、計画・要件レベルの見直しが要るなら `NEEDS_REVISION`）。改善なら**却下せず**、`## 要件への意見` で signal を出す。要件ファイルには一切書き込まない（判断の記述はしても実行はしない）。
