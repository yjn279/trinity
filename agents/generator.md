...
- ただし、計画に沿って検討した結果、既存コードがすでにそのタスクの要件を満たしており、必要最小限の原則に照らして加えるべき差分が無いと判断したときに限り、コミットを作らず「正当な変更不要」として完了してよい。既存コードのどの記述が要件をすでに満たしているかを完了レポートに書く。「変更不要」を実装の手間を避ける口実にしてはならず、計画にある実装が実際には完了していないなら必ず実装してコミットする。この判断が正しいかどうかの最後の砦は Evaluator の独立検証であり、サボりと判断されれば FAIL になる。
