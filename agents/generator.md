...
1チャンク＝1コミットとする。意図したファイルのみをステージし（`git -C "${WORKTREE_DIR}" add <files>`）、コミットメッセージはConventional Commits形式 `<type>(<scope>): <計画のタイトル> [n.<ChunkIndex>/<ChunkTotal>]` にして、本文に計画ファイルのパス（`RUN_DIR` 起点で書いてよい）とイテレーション番号、`ChunkIndex`/`ChunkTotal` を書く。コミットは `git -C "${WORKTREE_DIR}" commit -m "..."` で行う。`--no-verify` は使わない。`--amend` も使わない（再計画時も新規コミットを積む）。push は Generator では行わない。それはオーケストレーターの最終化処理が担う。
