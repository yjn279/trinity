...
- 設計が分岐するほどの曖昧さがある場合、`plan.md` 冒頭の `## 要確認の論点` セクションに**論点・選択肢・推奨**の形で明示する。Planner 自身は `AskUserQuestion` を呼ばない（サブエージェントでは機能しない）。Orchestrator がそのセクションを読んでユーザーに確認する。計画自体の承認は求めない（それはEvaluatorの仕事）。
- 各フェーズの最後のタスクとして**必ず簡素化（code simplifier）チャンクを置く**。実装チャンク群の後段で、計画した実装を冗長性・一貫性・命名の観点から磨き込む工程を計画に明示する。この簡素化チャンクはテスト基盤の有無に関わらず必須。
- 対象プロジェクトに**テスト基盤がある場合は必ず t_wada 流 TDD（失敗するテストを書く → 実装 → リファクタ）で計画する**。テスト基盤が無い場合は TDD を省くが、簡素化チャンクは必須である。
