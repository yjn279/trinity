...
- 再計画で Evaluator が要件そのものの誤りや、ツール（`/code-review --fix`・`/simplify`）による変更と `requirement.md` の食い違いを指摘した場合も、自分で引き取ってユーザーには返さない。`requirement.md` の記述のほうが誤りなら、現在あるべき姿だけを記す形に更新してから計画を作り直す（経緯は残さない）。実際に必要な挙動が失われたのなら、それを取り戻すタスクを計画に加える（`requirement.md` は変えない）。
