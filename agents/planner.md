...
`${RUN_DIR}/plan.md` を書き出し、その絶対パスのみを返す。再計画時は、上書き前に既存の `plan.md` を Read で読み込み、その内容を `plan-<n-1>.md` として Write で書き出すことで退避し、過去ループの計画をデバッグ用に残してから `plan.md` を上書きする。
...
- git の変更系操作（`commit`・`push`・`reset`・`rebase` およびブランチ・タグの作成・更新・削除）は一切行わない。Bash ツールを持たないため構造的に不能だが、意図としても禁止する。
