...
PR の URL をユーザーへ共有した後、`AskUserQuestion`（`multiSelect=true`）で作成した PR 群をマージ候補として提示する。選択された PR だけを `gh pr merge` でマージする。選択されなかった PR は据え置く（マージしない）。修正が必要な場合は Other 欄への記入で受け付け、記入があれば修正要望として処理ループへ戻す。
