...
- FR-O-4 `trinity-fanout` に `backlog.tsv` を渡し、起動可能なパイプラインを背景起動する。
- FR-O-5 `trinity-wait` を呼んでイベント（`needs-input`・`unblocked`・`done`・`timeout`）までブロックして待ち、各イベントに対応する。
...
- FR-H-2 `bin/trinity-pipeline` は1 Issue の収束ループを駆動する。`plan → generate → tools → evaluate` を最大 `TRINITY_MAX_LOOPS`（既定5）回繰り返し、`VERDICT` に応じて分岐する。
- FR-H-3 `bin/trinity-fanout` は `backlog.tsv` を読み、依存が満たされ worktree が用意できた Issue を、並列上限 `TRINITY_MAX_PARALLEL`（既定3）の範囲で背景起動する。受け渡し境界は `backlog.tsv` 一枚。
- FR-H-4 `bin/trinity-wait` は手当てが要るイベントまでブロックして待ち、`STATUS` 表と `EVENT:` 行を返す。`unblocked` は空きスロットがあるときだけ通知する。
- FR-H-5 パイプラインは状態を `RUN_DIR/status` に1語で出力する（`planning`・`generating`・`reviewing`・`evaluating`・`needs-input`・`queued`・`passed`・`needs-revision`・`failed`・`error`）。Orchestrator はこの1ファイルだけで進捗を把握できる。
- FR-H-6 `TRINITY_DRY_RUN=1` のとき `claude` を呼ばず制御フローだけを検証できる。
...
| 出力者 | 成果物 | 読む側 |
| :-- | :-- | :-- |
| Orchestrator | `SESSION_DIR/backlog.tsv` | `trinity-fanout`・`trinity-wait` |
...
- AC-5 `shellcheck -S warning` がクリーンで、`TRINITY_DRY_RUN=1` で `claude` を呼ばずに全分岐の制御フローを検証できる。
