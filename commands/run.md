...
- `MAX_ITER` トークンを取り除いた後の残り文字列を `RAW_TASK` として保持する（前後の空白を除去する）。`RAW_TASK` は「要件」と同一の値である
...
- `git-merge` スキル内のマージ可否確認 `AskUserQuestion`（実行手順 5c） → ユーザー回答を待つ
- PASS 後の起票候補ヒアリング `AskUserQuestion`（実行手順 5d / 5e） → ユーザー回答を待つ
...
2. **`TASK_DISPLAY` の組み立て**: worktree 作成直後、ループ開始前に 1 度だけ実行する。以降のすべての最終出力で使う表示文字列 `TASK_DISPLAY` を次の規範で組み立てる。

   1. `RAW_TASK` 全体（または先頭トークン）が GitHub issue URL `https://github.com/<owner>/<repo>/issues/<N>` 形式に正規表現マッチするか判定する（`<N>` は正の整数。トレイリングスラッシュやクエリ文字列は許容する）。
   2. マッチした場合、`<owner>` `<repo>` `<N>` を抽出して次のコマンドで issue タイトルを取得する。

      ```shell
      gh issue view <N> --repo <owner>/<repo> --json title --jq '.title'
      ```

      取得に成功したら `TASK_DISPLAY = "#<N> <title> (<URL>)"` の形にする。コマンドが失敗した場合（ネットワーク不通・認証なし・private repo 等）はフォールバックとして `TASK_DISPLAY = RAW_TASK` をそのまま使う。失敗の警告は最終出力以外に流出させない。
   3. マッチしない場合は `TASK_DISPLAY = RAW_TASK` をそのまま使う。
   4. `TASK_DISPLAY` は最終的に **1 行** に収める。改行（`\n` / `\r`）はスペースに置換する。120 文字（半角ベース。Unicode codepoint 単位）を超える部分は末尾を `…` に切り詰める（byte 切り詰めは禁止）。

   要件文中に複数の issue URL が混在する場合は最初に見つかった `issues/<N>` URL のみを参照する。issue 以外の GitHub URL（PR・commit・discussions 等）の解決はスコープ外とし、`RAW_TASK` をそのまま使う。

3. **ループ**: `n = 1 .. MAX_ITER` で次を順に呼ぶ。並列にしない。
...
5. **PASS のとき**: 次を順に実行する。
   ...
   **d. PASS 後の起票候補ヒアリング（ユーザープロジェクト向け）**
   ...
   PR 作成（5b）が成功した経路では、5c の結果（マージ・クローズ・needs-revision）にかかわらず本ステップを実施する。詳細は後述「PASS後の起票候補ヒアリング（post-run-issue-suggestions）」に従う。`AskUserQuestion` を 1 コール呼び（候補数に応じて 1〜4 問に分割）、選択された候補は即 `gh issue create` する。起票結果は最終出力（ステップ 8）の `Issues:` セクションに反映する。

   **e. PASS 後の trinity プラグイン自身バグ・要望ヒアリング**

   ステップ 5d の直後、ステップ 7/8 の直前に実施する。詳細は後述「PASS後 trinity プラグイン自身バグ・要望ヒアリング（post-run-trinity-self-issue-suggestions）」に従う。起票先リポジトリは `yjn279/trinity` でハードコード。起票結果は最終出力の `Issues:` セクションに反映する。

6. **PASS でない / `MAX_ITER` 到達**: 最終化をスキップし、最新の評価レポートのパスと未解決の指摘をユーザーに表示して停止。push も PR 作成もマージ確認もクリーンアップも起票候補ヒアリングも行わない。

7. **ログ**: ループ脱出時または `MAX_ITER` 到達時に `.trinity/trinity.log` に終了行を追記する。
...
8. **最終出力**: 次のフォーマットでちょうど印字し、2〜3 文の要約を添える。最終化を実施した（PASS で抜けた）場合だけ `PR:` `Merge:` `Cleanup:` の 3 行を加える。`Issues:` セクションは PASS かつ起票が 1 件以上発生したときだけ加える。`Task:` 行は PASS のときも、NEEDS_REVISION / FAIL で `MAX_ITER` に到達したときも、**常に** 出力する（`PR:` `Merge:` `Cleanup:` のように PASS 限定ではない）。
...
本段は PASS で `git-pull-request` スキル経由の PR 作成（実行手順 5b）が成功したあと、ステップ 5e と 7/8 の前に実施する（実行手順 5d）。
...
起票が1件以上発生したとき、最終出力（実行手順 8）の `Issues:` セクションに各 issue URL を1行ずつ列挙する。

## PASS後 trinity プラグイン自身バグ・要望ヒアリング（post-run-trinity-self-issue-suggestions）

### 位置

本段は前段 `post-run-issue-suggestions`（実行手順 5d）が終わった直後・実行手順 7/8（ログ・最終出力）の直前に実施する（実行手順 5e）。前段とは別段として共存し、前段が存在しなくても本段は単体で成立する。
...
起票が 1 件以上発生したとき、最終出力（実行手順 8）の `Issues:` セクション内に「trinity プラグイン向け」の連続ブロックとして各 issue URL を 1 行ずつ列挙する。前段 `post-run-issue-suggestions` の起票結果（ユーザープロジェクト向け）と並列に並ぶ。
