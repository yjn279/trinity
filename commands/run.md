...
- 残りのトークンを確認し、`--pr=<VALUE>` 形式のトークンが存在するかを検査する
  - `--pr=<VALUE>` が 1 つある場合: `PR_REF = <VALUE>` とし、そのトークンを取り除く。`MODE = pr-continuation`
  - `--pr=<VALUE>` が 2 つ以上ある場合: エラーで停止し、ユーザーに「`--pr` を 2 回以上指定しています。1 回だけ指定してください」と報告する（指定重複は指定漏れと同様の事故防止）
  - `--pr=<VALUE>` が 0 個の場合: `MODE = default`
