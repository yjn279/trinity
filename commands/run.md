...
ハーネスを取り回すスラッシュコマンドである。Planner が要件を計画に展開し、Generator が隔離された worktree で実装してコミットし、Evaluator が独立に判定する。判定が PASS になるか、`max_iter` に到達するまで繰り返す。最終 PASS 後、worktree のブランチを push して PR を作成し（既存 PR 上で動いていた場合は追加 push のみ）、ユーザー承認のもとでマージとクリーンアップまでを行う。

新規に始めるか既存 PR を継続するかは、ユーザーがフラグで指定するのではなく、コマンド側がリポジトリの状態を見て自動的に判断する。判断ロジックは「実行手順 1: 準備」で定義する。

## 使うスキル

このコマンドの実行手順は次の 3 スキルに分割されている。各フェーズで該当スキルを参照し、その手順に従う。スキル本文を要約せず、書かれている規範をそのまま守ること。

- `git-worktree` — 起動直後、要件由来のスラッグを渡して隔離 worktree を作成する。新規に始める場合はスラッグだけを渡し、`origin/<デフォルトブランチ>` を base とした新規ブランチが切られる。既存 PR を継続する場合はスラッグと `PR_NUMBER` を渡し、その PR の head ブランチが worktree にチェックアウトされる。`TS` / `SLUG` / `BRANCH` / `RUN_DIR` / `WORKTREE_DIR` / `BASE_BRANCH` はスキルが返す
- `git-pull-request` — PASS 後の origin への push と PR の確定を一連フローで実行する。新規 PR の場合は PR タイトルと PR 本文を渡して新規作成する。既存 PR の継続の場合は `PR_NUMBER` を渡して追加 push のみを行い、新規 PR は作成しない。`PR_NUMBER` / `PR_URL` を返す
- `git-merge` — マージ可否確認の `AskUserQuestion` から、承認時のマージ・後始末、否認時の改善項目ヒアリングまでを内蔵する。呼び出し側から渡すのは PR URL のみ。新規作成した PR でも継続した既存 PR でも同一フローで動く
...
1. **準備**: `--resume` の有無で次の 2 経路に分岐する。
   ...
   **通常起動（`--resume` なし）**: まず「現在のリポジトリ状態から既存作業を自動判断する」（下記参照）ことで `PR_NUMBER`（既存 PR を継続するなら確定値、しないなら未設定）を決め、その結果に従って `git-worktree` スキルを呼んで隔離 worktree を作成する。

   呼び出し側で要件文または継続対象の PR タイトルから kebab-case のスラッグ（2〜5 語）を生成する。リポジトリパス・ベースブランチ・ログファイルパスはスキル内で推測する。

   - `SLUG` = 既存 PR を継続しない場合は要件文から派生させた kebab-case スラッグ（例: 「ユーザー設定ページにテーマトグルを追加する」→ `add-theme-toggle`）。既存 PR を継続する場合は `gh pr view <PR_NUMBER>` で取得した PR タイトルから派生させた kebab-case スラッグ

   スキルから次の値を受け取り、以降のすべてのフェーズで使う。

   - `TS` — タイムスタンプ
   - `SLUG` — 確定した slug（衝突時は `-2` `-3` を付けて一意化された値）
   - `BRANCH` — ブランチ名（新規経路では `trinity/<TS>-<SLUG>` 形式。既存 PR 継続経路では PR の head ブランチ名）
   - `RUN_DIR` — run ディレクトリの絶対パス
   - `WORKTREE_DIR` — worktree の絶対パス
   - `BASE_BRANCH` — base ブランチ名
   - `PR_NUMBER` / `PR_URL` — 既存 PR を継続する場合のみ返る

   ### 現在のリポジトリ状態から既存作業を自動判断する

   `--resume` なしで起動した時点で、コマンド側が次の手順でリポジトリ状態を読み取り、`PR_NUMBER` を確定するか「未設定（新規ブランチで始める）」を確定する。ユーザーがフラグで切り替える代わりに、状態から自然に決まる。
   ...
   **判定が持つ安全側の意味**:
   - 決定木 #1: `CURRENT_BRANCH == DEFAULT_BRANCH`（典型的には `main`）のときは無条件で新規ブランチを切る。`main` 上での実行が必ず新規ブランチを切る保証となり、意図せず既存 PR を上書きする事故を防ぐ。
   - 決定木 #2: `trinity/` プレフィックスを持たないブランチは、たとえそのブランチに紐づく open PR があっても新規ブランチを切る。Trinity 起源でないブランチ上の PR への暗黙的な追加 push を防ぐ。
   - 決定木 #5: 同一ブランチに open PR が複数紐付いている曖昧ケースでは停止し、ユーザーに状況を提示して再実行を促す。

   **`git-worktree` スキルの呼び出し**:
   - 新規ブランチで始める場合: スラッグのみをスキルに渡す。スキルは `origin/<デフォルトブランチ>` を base として新規ブランチを切る。
   - 既存 PR を継続する場合: スラッグと `PR_NUMBER` をスキルに渡す。スキルは PR の head ブランチを worktree にチェックアウトする（新規ブランチは作らない）。
...
4. **PASS のとき**: 実行手順 1 で確定した `PR_NUMBER` の有無に応じて次を順に実行する。
   ...
   **a. PR タイトルと PR 本文を組み立てる（新規 PR を作る場合のみ）**

   既存 PR を継続する場合は PR 本文を再構築しない（追加 push のみを行い PR 本体は更新しない）。そのため 4a は新規 PR を作る場合（`PR_NUMBER` が未設定）のみ実行する。
   ...
   **b. `git-pull-request` を呼ぶ**
   ...
   `PR_NUMBER` の有無に応じて呼び方が異なる。

   **新規 PR を作る場合（`PR_NUMBER` が未設定）**:

   組み立てた PR タイトルと PR 本文をスキルに渡す。worktree パス・ブランチ名・ベースブランチ・リモート情報はスキル内で推測する。
   ...
   **既存 PR を継続する場合（`PR_NUMBER` が確定済み）**:

   PR タイトル・PR 本文は渡さない。`PR_NUMBER` のみをスキルに渡す。スキルは新規 PR を作成せず、既存 PR のブランチへ追加 push するだけである。

   - `PR_NUMBER` = 実行手順 1 の自動判断で確定した PR 番号

   返ってきた `PR_URL` を保持する（`PR_NUMBER` は入力と同値）。

   **c. `git-merge` を呼ぶ**

   PR URL だけをスキルに渡す。マージ可否確認の `AskUserQuestion`、承認時のマージ・後始末（リモート/ローカルブランチ削除、worktree 削除、run ディレクトリ削除）、否認時の改善項目ヒアリングまで、すべてスキル内で完結する。ブランチ名・worktree パス・run ディレクトリのパス・リポジトリパスはスキル内で推測する。新規 PR でも既存 PR の継続でも、スキルの動作は不変である。
   ...
   **d. PASS 後の起票候補ヒアリング（ユーザープロジェクト向け）**
   ...
   PR 確定（4b）が成功した経路では、4c の結果（マージ・クローズ・needs-revision）にかかわらず本ステップを実施する。新規 PR でも既存 PR の継続でも同じく実施する。詳細は後述「PASS後の起票候補ヒアリング（post-run-issue-suggestions）」に従う。`AskUserQuestion` を 1 コール呼び（候補数に応じて 1〜4 問に分割）、選択された候補は即 `gh issue create` する。起票結果は最終出力（ステップ 7）の `Issues:` セクションに反映する。
...
7. **最終出力**: 次のフォーマットでちょうど印字し、2〜3 文の要約を添える。最終化を実施した（PASS で抜けた）場合だけ `PR:` `Merge:` `Cleanup:` の 3 行を加える。`Issues:` セクションは PASS かつ起票が 1 件以上発生したときだけ加える。`Task:` 行は PASS / NEEDS_REVISION / FAIL のいずれの終了経路でも無条件で出す。要件文の表示前に、連続する空白文字（半角・全角スペース、タブ、改行）を半角スペース 1 個に畳み、先頭・末尾の空白を取り除いた上で、200 文字（コードポイント単位）を超える場合は末尾を `…`（U+2026）で切る。
   ...
   `PR:` 行は新規作成・既存 PR への追加 push のどちらでも `#<N> <URL>` の同一フォーマットを使う。新規か継続かの区別は出力には出さない（PR の URL を見れば分かる）。
   ...
   NEEDS_REVISION / FAIL のまま `MAX_ITER` に到達した場合は `PR:` `Merge:` `Cleanup:` `Issues:` 行を出さない。`Task:` 行は経路にかかわらず常に出す。

`/trinity:run` の起動自体がパイプライン全体への明示的な許可（push と PR 作成または既存 PR への追加 push、起票候補ヒアリングを含む）だが、マージとクリーンアップだけは PR 確定後に `git-merge` スキルが呼ぶ `AskUserQuestion` で改めてユーザーの承認を取る。途中で他の確認プロンプトは挟まない。
...
本段は PASS で `git-pull-request` スキル経由の PR 確定（実行手順 4b）が成功したあと、ステップ 4e と 6/7 の前に実施する（実行手順 4d）。新規 PR でも既存 PR の継続でも、PR 確定が成功した経路で本段を実施する。
...
PASS で `git-pull-request` スキル経由の PR 確定が成功した最終化経路でのみ実施する。
