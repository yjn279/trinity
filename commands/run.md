---
description: "Planner → Generator → Evaluator のハーネスパイプラインを実行する。最終 PASS 後は push と PR 作成を行い、ユーザー承認のもとでマージとクリーンアップまでを完結させる。使用例 `/trinity:run <要件>` または `/trinity:run --max-iter=5 <要件>`。既存 PR への追加コミット運用は `/trinity:run --pr=<NUMBER|URL> <要件>` で指定する。"
argument-hint: "[--max-iter=N] [--pr=<NUMBER|URL>] <1〜4文の要件>"
---
...
- `git-worktree` — 起動直後、要件由来のスラッグ（と PR continuation モードでは `PR_REF`）を渡して隔離 worktree を作成する。TS / SLUG / BRANCH / RUN_DIR / WORKTREE_DIR / BASE_BRANCH はスキルが返す
- `git-pull-request` — PASS 後の origin への push と PR 作成（または PR continuation モードでは push のみ）を一連フローで実行する。呼び出し側から渡すのは PR タイトルと PR 本文（および PR continuation モードでは MODE と EXISTING_PR_URL）で、`PR_NUMBER` / `PR_URL` を返す
...
- `--max-iter=N` を取り除いた後の先頭トークンが `--pr=<VALUE>` にマッチするかを確認する
  - **マッチする場合**: `PR_REF = <VALUE>` とし、そのトークンを取り除く。`MODE = pr-continuation`。取り除いた直後の先頭トークンが再度 `--pr=<VALUE>` にマッチするなら重複指定としてエラーで停止し、ユーザーに「`--pr` を 2 回以上指定しています。1 回だけ指定してください」と報告する
  - **マッチしない場合**: `MODE = default`。残りのトークン列を全体スキャンして `--pr=<VALUE>` を探す方式は採らない（要件文中に偶然 `--pr=` を含む文字列が現れたときの誤検出を防ぐため、また `--max-iter=N` の解釈と一貫させるため）
- 残りを「要件」として扱う。要件が空ならユーザーに 1〜4 文の要件を求めて停止する

### PR_REF の解決（MODE=pr-continuation のみ）

`PR_REF` が指定された場合、**要件のループを開始する前に**次のコマンドで PR 情報を解決する。

```shell
# PR_REF が整数のとき（例: --pr=90）
gh pr view "$PR_REF" --json headRefName,baseRefName,url,state,number

# PR_REF が URL のとき（例: --pr=https://github.com/owner/repo/pull/90）
gh pr view "$PR_REF" --json headRefName,baseRefName,url,state,number
```

整数・URL のどちらでも `gh pr view` に渡してよい（`gh` は両方を受理する）。
...
| --- | --- |
| `PR_NUMBER` | PR 番号（整数） |
| `PR_URL` | 正規化された PR URL |
| `PR_HEAD_BRANCH` | PR の head ブランチ名 |
| `PR_BASE_BRANCH` | PR の base ブランチ名 |

解決に失敗した場合は**処理を開始せず停止し**、ユーザーに次のいずれかを表示する。

- PR が存在しない: `--pr=<VALUE> の PR が見つかりません。番号または URL を確認してください。`
- PR が closed / merged: `PR #<N>（<URL>）は <state> 状態です。open な PR のみ指定できます。`

起動例:

```
/trinity:run --pr=90 レビュアーコメント反映: エラーハンドリングを追加する
/trinity:run --max-iter=3 --pr=https://github.com/owner/repo/pull/90 型定義を修正する
```
...
このため、本コマンドが起動した時点で「現在のブランチが clean なベースライン」であることが保証されている。このチェックは `MODE = default` でも `MODE = pr-continuation` でも同様に適用される。
...
1. **準備**: `git-worktree` スキルを呼んで隔離 worktree を作成する。
   ...
   呼び出し側で要件文から kebab-case のスラッグ（2〜5 語）を生成する。

   - `SLUG` = 要件文から派生した kebab-case のスラッグ（例: 「ユーザー設定ページにテーマトグルを追加する」→ `add-theme-toggle`）

   `MODE` に応じてスキルへの渡し方が変わる。

   | MODE | スキルへ渡す引数 |
   | --- | --- |
   | `default` | スラッグのみ |
   | `pr-continuation` | スラッグ + `PR_REF`（引数解釈で確定した値） |

   スキルから次の値を受け取り、以降のすべてのフェーズで使う。

   - `TS` — タイムスタンプ
   - `SLUG` — 確定した slug（衝突時は `-2` `-3` を付けて一意化された値）
   - `BRANCH` — ブランチ名（`default` では `trinity/<TS>-<SLUG>` 形式、`pr-continuation` では PR の `headRefName`）
...
4. **PASS のとき**: 次を順に実行する。
   ...
   **b. `git-pull-request` を呼ぶ**
   ...
   | MODE | スキルへ渡す引数 | スキルの動作 |
   | --- | --- | --- |
   | `default` | `PR_TITLE` + `PR_BODY` | push + `gh pr create` で新規 PR 作成 |
   | `pr-continuation` | `PR_TITLE` + `PR_BODY` + `MODE=pr-continuation` + `EXISTING_PR_URL=<PR_URL>` | push のみ。`gh pr create` は呼ばない |

   返ってきた `PR_URL` を保持する（`pr-continuation` モードでは渡した `EXISTING_PR_URL` がそのまま返る）。

   **b-2. PR continuation モードのコメント投稿**

   `MODE = pr-continuation` のとき、上記 4a で組み立てた本文（Trinity 実行サマリを含む）を、新規 PR の本文としてではなく、既存 PR へのコメントとして投稿する。

   ```shell
   gh pr comment "$PR_URL" --body "$BODY"
   ```

   `gh pr edit` で既存 PR 本文を上書きしない（レビュアーが書いた既存本文を破壊するリスクを避けるため）。

   コメント投稿に失敗した場合は失敗を表示してユーザーに委ねる。push 自体が成功していれば最終化の主目的は満たしているため、コメント投稿失敗は致命的エラーとして扱わない。

   **c. `git-merge` を呼ぶ**

   PR URL だけをスキルに渡す。`MODE` に関係なく、スキルへの入力は `PR_URL` 1 つのみ。マージ可否確認の `AskUserQuestion`、承認時のマージ・後始末（リモート/ローカルブランチ削除、worktree 削除、run ディレクトリ削除）、否認時の改善項目ヒアリングまで、すべてスキル内で完結する。ブランチ名・worktree パス・run ディレクトリのパス・リポジトリパスはスキル内で推測する（`BRANCH` が `trinity/<TS>-<SLUG>` 形式でなくても動作する）。

   - `PR_URL` = 上で取得した値（`default` なら新規作成 URL、`pr-continuation` なら既存 PR の URL）
   ...
   **d. PASS 後の起票候補ヒアリング（ユーザープロジェクト向け）**
   ...
   PR 作成または PR continuation モードでの push（4b）が成功した経路では、4c の結果（マージ・クローズ・needs-revision）にかかわらず本ステップを実施する。詳細は後述「PASS後の起票候補ヒアリング（post-run-issue-suggestions）」に従う。`AskUserQuestion` を 1 コール呼び（候補数に応じて 1〜4 問に分割）、選択された候補は即 `gh issue create` する。起票結果は最終出力（ステップ 7）の `Issues:` セクションに反映する。
...
7. **最終出力**: 次のフォーマットでちょうど印字し、2〜3 文の要約を添える。最終化を実施した（PASS で抜けた）場合だけ `PR:` `Merge:` `Cleanup:` の 3 行を加える。`Issues:` セクションは PASS かつ起票が 1 件以上発生したときだけ加える。
   ...
   Mode:    <default | pr-continuation (existing PR #<PR_NUMBER>)>
   RunDir:  <RUN_DIR>
   ...
   Iters:   <n>/<MAX_ITER>
   PR:      #<PR_NUMBER> <PR_URL>                                        # PASS のときのみ（default）
   PR:      #<PR_NUMBER> <PR_URL> (continuation)                         # PASS のときのみ（pr-continuation）
   ...
   `Mode:` 行は常に出力する（PASS でない場合も含む）。`PR:` 行は PASS 時のみ出す。`pr-continuation` モードでは `(continuation)` サフィックスを末尾に付け、新規作成ではなく既存 PR への追加 push であったことを示す。
...
本段は PASS で `git-pull-request` スキル経由の push（既定モードでは push + PR 作成、PR continuation モードでは push のみ）が成功したあと、ステップ 4e と 6/7 の前に実施する（実行手順 4d）。
...
PASS で `git-pull-request` スキル経由の push が成功した最終化経路でのみ実施する。
