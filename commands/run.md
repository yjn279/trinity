---
description: "Planner → Generator → Evaluator のハーネスパイプラインを実行する。最終 PASS 後は push と PR 作成を行い、ユーザー承認のもとでマージとクリーンアップまでを完結させる。使用例 `/trinity:run <要件>` または `/trinity:run --max-iter=5 <要件>` または `/trinity:run --resume=<RUN_DIR>` または `/trinity:run --pr=<PR_NUMBER> <要件>`。"
argument-hint: "[--max-iter=N] [--resume=<RUN_DIR>] [--pr=<PR_NUMBER>] <1〜4文の要件>"
---
...
- `git-worktree` — 起動直後、要件由来のスラッグだけ渡して隔離 worktree を作成する。通常モードでは `origin/<デフォルトブランチ>` を base として新規ブランチを切る。`--pr=<N>` 経路では `PR_NUMBER` を渡して PR の head ブランチを worktree にチェックアウトする。TS / SLUG / BRANCH / RUN_DIR / WORKTREE_DIR / BASE_BRANCH / MODE はスキルが返す
...
- `--pr=<N>`（N は正の整数）が含まれていれば `PR_NUMBER = N` とし「既存 PR モード」で動作する（後述）
- `--resume` と `--pr` が**同時に指定された場合は停止**し、次の案内を表示してから終了する:
  > `--resume` と `--pr` は同時に使えません。`--resume` は Trinity が以前作った run の続きを再開する経路、`--pr` は既存の PR に新規ループの成果物を追加 push する経路です。どちらを使うか確認した上で再実行してください。
- `MAX_ITER` は 1 未満を受け付けない。0 以下なら停止して報告する
- 残りのトークンを「要件」として扱う。`--resume` も `--pr` も含まない場合、要件が空ならユーザーに 1〜4 文の要件を求めて停止する
- `--pr` がある場合に要件文が空のときは停止し、「`--pr` 経路でも要件文は必須です。1〜4 文の要件を添えて再実行してください」とユーザーに告げる（既存 PR の最新コメントから要件を自動抽出しない）
- `--pr` あり・要件文ありの場合: 要件文を Planner にそのまま渡す。Planner は新規に `plan.md` を書く（`--resume` と異なり既存 plan を引き継がない）
...
1. **準備**: `--resume` / `--pr` の有無で次の 3 経路に分岐する。

   **通常起動（`--resume` なし、`--pr` なし）**: `git-worktree` スキルを呼んで隔離 worktree を作成する。

   呼び出し側で要件文から kebab-case のスラッグ（2〜5 語）を生成し、それだけをスキルに渡す。リポジトリパス・ベースブランチ・ログファイルパスはスキル内で推測する。スキルは `origin/<デフォルトブランチ>` を base として worktree を作成する（現在の作業ブランチには依存しない）。
   ...
   - `BASE_BRANCH` — base ブランチ名（`origin/<デフォルトブランチ>` から導出した値。例: `main`）
   - `MODE` — `new-branch`（通常起動時の固定値）
   ...
   **既存 PR モード（`--pr=<N>` あり）**: `git-worktree` スキルを `PR_NUMBER` 付きで呼ぶ。スキルは PR #N の head ブランチを worktree にチェックアウトし、新規ブランチを作らない。

   - `SLUG` = `gh pr view <N>` で取得した PR タイトルから派生した kebab-case のスラッグ（呼び出し側で事前に生成する）

   スキルから次の値を受け取り、以降のすべてのフェーズで使う。

   - `TS` — タイムスタンプ
   - `SLUG` — 確定した slug
   - `BRANCH` — PR の head ブランチ名（新規ブランチは作らない）
   - `RUN_DIR` — run ディレクトリの絶対パス
   - `WORKTREE_DIR` — worktree の絶対パス
   - `BASE_BRANCH` — PR の base ブランチ名（PR の base に合わせる）
   - `MODE` — `existing-pr`
   - `PR_NUMBER` — 引数で渡した PR 番号
   - `PR_URL` — PR の URL
...
4. **PASS のとき**: `MODE` に応じて次を順に実行する。
   ...
   **`MODE=existing-pr` の場合（既存 PR への追加 push のみ）**:
   ...
   - `MODE` = `existing-pr`
   - `PR_NUMBER` = 引数で渡した PR 番号（実行手順 1 で取得済み）
