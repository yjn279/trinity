...
Planner が要件を計画に展開し、Generator が隔離された worktree で実装してコミットし、Evaluator が独立に判定する。PASS になるか `max_iter` に到達するまで繰り返す。PASS 後、worktree のブランチを push して PR を作成し（既存 PR 上で動いていた場合は追加 push のみ）、ユーザー承認のもとでマージとクリーンアップまでを行う。

新規に始めるか既存 PR を継続するかは、コマンド側がリポジトリの状態を見て自動的に判断する（`PR_NUMBER` が確定すれば既存 PR 継続、未設定なら新規）。判断ロジックは「実行手順 1: 準備」で定義する。
...
サブエージェントは並列ではなく直列に呼び出す。各段は前段の成果ファイル（`plan.md` / コミット / `eval-<n>.md`）に依存している。Evaluator はコミット SHA を入力に取るので Generator が終わってからしか起動できない。並列化すると存在しない SHA を渡してしまう。同一イテレーション内チャンクも同様に直列（並列起動はしない）。

### 段と段のあいだでコードに触らない

オーケストレーターが `Read` `Edit` `Bash` で `${WORKTREE_DIR}` 内のコードを読んだり編集したりしてはいけない。例外なく禁止する。触ってよいのは:
...
最終出力フォーマット 1 ブロック + 2〜3 文の要約だけを返す。例外:
...
4. **PASS のとき**: 実行手順 1 で確定した `PR_NUMBER` の有無に応じて次を順に実行する。
   ...
   **a. PR タイトルと PR 本文を組み立てる（`PR_NUMBER` 未設定時のみ）**
   ...
   PR タイトルは `${RUN_DIR}/plan.md` の先頭 H1 をそのまま使う（70 文字超なら冒頭で切り詰める）。PR 本文（`.trinity/` は gitignore されておりレビュアーから見えないため計画と判定の核心を本文に埋め込む）:
...
7. **最終出力**: 次のフォーマットでちょうど印字し、2〜3 文の要約を添える。PASS で抜けた場合のみ `PR:` `Merge:` `Cleanup:` を加える。`Task:` 行は経路によらず常に出す。NEEDS_REVISION / FAIL で `MAX_ITER` 到達なら `PR:` `Merge:` `Cleanup:` `Issues:` を出さない。`Issues:` は PASS かつ起票が 1 件以上のときのみ加え、ユーザープロジェクト向けと trinity プラグイン向けの 2 連続ブロックで並べる（各ブロックは 1 件以上のときだけ出す。両方 0 件なら行自体を出さない）。

   `PR:` 行は `#<N> <URL>` フォーマット。要件文は連続する空白を半角スペース 1 個に畳み、先頭・末尾を除き 200 文字超なら末尾を `…`（U+2026）で切る。
...
`/trinity:run` の起動自体がパイプライン全体への明示的な許可（push・PR 作成または追加 push・起票候補ヒアリングを含む）だが、マージとクリーンアップだけは `git-merge` スキルが呼ぶ `AskUserQuestion` で改めてユーザーの承認を取る。途中で他の確認プロンプトは挟まない。
...
両段（実行手順 4d / 4e）は同じ構造で動く。差分は候補抽出元・起票先 repo・スキップファイル名・`Issues:` 内の並び位置のみ（各段の「差分」節を参照）。PASS で PR 確定が成功した経路でのみ実施し、`NEEDS_REVISION` / `FAIL` で `max_iter` に達した場合はスキップする。

**候補の整理**: 各候補を 2 要素（タイトル候補 = issue タイトル 1 文 / 本文候補 = 背景・期待動作・関連ファイルパス等）に分けて整理する。候補が 16 を超える場合は重要度上位 16 件に絞り、残りはスキップファイルにメモする（ファイル名は各段で異なり、衝突は発生しない）。

**ユーザーへの提示（AskUserQuestion）**: 候補が 1 件以上ある場合は `AskUserQuestion`（`multiSelect=true`）を **1コール** 呼ぶ。候補数に応じて次のルールで問に分割する。
...
1 問あたり最大 4 オプション、1 コールあたり最大 4 問。ラベル＝タイトル候補、description＝本文候補の短い要約。「Other」は自動付与のためテンプレートに含めない。

**同意 = 即起票**: 選択された候補は追加確認プロンプトを挟まず即 `gh issue create` を 1 件ずつ連続実行する。

```shell
gh issue create --repo <owner/repo> --title "<title>" --body "<body>"
```

**拒否候補の保存**: 選択されなかった候補はスキップファイルに「タイトル候補 / 本文候補 / 抽出元（`eval` / `gen NOTES` / `PR スコープ外`）」が見て取れる Markdown 形式で書き出す。

**起票結果**: 起票が 1 件以上発生したとき `Issues:` セクションに各 issue URL を列挙する。候補が 0 件のときは `AskUserQuestion` を呼ばず、起票結果も出さない。
...
本段は実行手順 4d の直後・実行手順 6/7 の直前（実行手順 4e）。前段が存在しなくても単体で成立する。共通構造は前段の「共通構造」節を参照。

### 候補の抽出（post-run-trinity-self-issue-suggestions 固有）

本 run 中の観察・`${RUN_DIR}/eval-<n>.md`・Generator NOTES の中から **trinity プラグイン本体（リポジトリ `yjn279/trinity`）の挙動に起因する** ものを拾う。ユーザーのプロジェクトコードに起因するものは前段に任せる。

trinity プラグイン本体に起因する例: skill 指示が曖昧でエージェントが解釈を迷った / hook が期待どおり起動しなかった / `agents/*.md` の指示が冗長で解釈が割れた / `commands/run.md` の既存ステップに矛盾があった。

### 差分（post-run-trinity-self-issue-suggestions 固有）

- **起票先 repo**: `yjn279/trinity` でハードコード（`git remote get-url origin` 等の動的解決は使わない）

  ```shell
  gh issue create --repo yjn279/trinity --title "<title>" --body "<body>"
  ```

- **スキップファイル**: `${RUN_DIR}/skipped-trinity-self-suggestions.md`（前段の `${RUN_DIR}/skipped-suggestions.md` と異なり、ファイル名の衝突は発生しない）
- **`Issues:` 内の位置**: 「trinity プラグイン向け」ブロック（後）。前段の起票結果（ユーザープロジェクト向け）と並列に並ぶ
