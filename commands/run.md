...
- マージ可否確認 `AskUserQuestion`（実行手順 4b） → ユーザー回答を待つ
...
4. **PASS のとき**: 次を順に実行する。
   ...
   **b. マージ可否確認（`AskUserQuestion` — 本コマンドの責務）**
   ...
   `AskUserQuestion` を 1 回目として呼ぶ。本ステップ 4b 内での `AskUserQuestion` 呼び出し回数は最大 2 回（マージ確認 1 回 + 否認時ヒアリング 1 回）に制限する。後続ステップ 4d / 4e は本制限の対象外で、それぞれ自分の `AskUserQuestion` 呼び出しを持つ。
   ...
   **否認時**: `git-merge` を呼ばない。`AskUserQuestion` を 2 回目として 1 回だけ呼び、改善項目をヒアリングする。
   ...
   ユーザーの回答を受け取ったら、`Followup: <回答>` として最終出力に反映する。本ステップ 4b ではこれ以上 `AskUserQuestion` を呼ばない。
   ...
   **d. PASS 後の起票候補ヒアリング（ユーザープロジェクト向け）**
   ...
   PR 作成（4a）が成功した経路では、4b/4c の結果（承認・否認）にかかわらず本ステップを実施する。詳細は後述「PASS後の起票候補ヒアリング（post-run-issue-suggestions）」に従う。`AskUserQuestion` を 1 コール呼び（候補数に応じて 1〜4 問に分割）、選択された候補は即 `gh issue create` する。起票結果は最終出力（ステップ 7）の `Issues:` セクションに反映する。
...
`/trinity:run` の起動自体がパイプライン全体への明示的な許可（push と PR 作成、起票候補ヒアリングを含む）だが、マージとクリーンアップだけは PR 作成後の `AskUserQuestion` で改めてユーザーの承認を取る。途中で他の確認プロンプトは挟まない。
...
本段は PASS で `git-pull-request` スキル経由の PR 作成（実行手順 4a）が成功したあと、ステップ 4e と 6/7 の前に実施する（実行手順 4d）。
