...
生の引数は `$ARGUMENTS` で受け取る。次の手順で解釈する。フラグトークンは任意の順序で書けるので、順序に依存せず全トークンを走査してフラグを抽出する。

- トークン列を左から走査し、`--max-iter=N`（N は正の整数）があれば `MAX_ITER = N` とし、そのトークンを消費する。なければ `MAX_ITER = 15`（既定値）
- トークン列を走査し、`--resume=<PATH>` があれば `RESUME_PATH = <PATH>` とし、そのトークンを消費する。なければ `RESUME_PATH` は未設定
- フラグトークン（`--max-iter=...` / `--resume=...`）を消費した残りを「要件」として扱う
- `--resume` が指定されていない通常起動の場合は、要件が空ならユーザーに 1〜4 文の要件を求めて停止する
- `--resume` が指定されている場合は、要件が空でもエラーにしない（既存の `plan.md` をそのまま再利用するため、要件の再投入は不要）
- `--resume` が指定されている場合に要件文が与えられても、Planner に再投入せず既存の `plan.md` を再利用する（要件文を Planner に再投入したいユーザーは resume を使わず通常起動するべきである）
...
`hooks/hooks.json` の `UserPromptSubmit` hook は resume 経路でも変更しない。hook が守る「ユーザーがいるベースチェックアウト（BASE_BRANCH）が clean」という不変条件は、resume だろうと新規起動だろうと同じ意味を持つ。resume 経路で追加で守る不変条件（worktree の clean 検査・ブランチ整合性検査）は、下記「実行手順 1」の中で Orchestrator が明示的に検証する。
...
1. **準備**: `RESUME_PATH` の有無で 2 通りに分岐する。

   **通常起動（`--resume` なし）**

   `git-worktree` スキルを呼んで隔離 worktree を作成する。
   ...
   **resume 起動（`--resume=<PATH>` あり）**

   `git-worktree` スキルは呼ばない。既存の `RUN_DIR` と worktree をそのまま使う。

   まず `RESUME_PATH` を解決する。絶対パスであればそのまま使い、絶対パスでなければリポジトリルート（`git rev-parse --show-toplevel` で取得）からの相対パスとして解決する。

   次の検証を順に実施し、いずれかが失敗したら対応するエラーメッセージを出力して停止する。

   - `RESUME_PATH` が存在するディレクトリでなければ停止する（例: `trinity: --resume target not found: /abs/path`）
   - `${RESUME_PATH}/worktree` がディレクトリとして存在しなければ停止する（例: `trinity: --resume target has no worktree/ subdirectory`）
   - `git -C "${RESUME_PATH}/worktree" status --porcelain` の出力が空でなければ（dirty）停止し、uncommitted な変更があることをユーザーに伝える（commit / stash / discard を手で判断してから再度 resume するよう促す）
   - `git worktree list --porcelain` で `${RESUME_PATH}/worktree` に bind されているブランチ名を確認し、worktree 内の HEAD ブランチと一致しなければ整合性エラーで停止する（worktree が外的に操作された可能性がある旨を伝える）

   検証が通ったら次の変数を復元して以降のすべてのフェーズで使う。

   - `RUN_DIR` = `RESUME_PATH` の絶対パス
   - `WORKTREE_DIR` = `${RUN_DIR}/worktree`
   - `BRANCH` = `git -C "${WORKTREE_DIR}" rev-parse --abbrev-ref HEAD` で取得
   - `BASE_BRANCH` = `git -C "${WORKTREE_DIR}" rev-parse --abbrev-ref HEAD@{upstream}` で取得（取得できなければ `main` を既定とする）
   - `RUN_NAME` = `${RUN_DIR}` の basename（`<TS>-<SLUG>` の形式）

   変数復元後、再開地点の決定を行う（後述「実行手順 2: resume 経路の再開地点決定」に従う）。

2. **ループ**: 通常起動では `n = 1` から、resume 起動では後述の再開地点決定で確定した `n` から始め、`n .. MAX_ITER` で次を順に呼ぶ。並列にしない。`MAX_ITER` の意味は常に「ループ全体の上限イテレーション数」であり、resume 後の残り回数に再解釈しない（例: `MAX_ITER=15` で `n=11` から再開した場合は残り `11..15` の 5 回が上限）。

   **resume 起動の再開地点決定**

   resume 経路に入ったら、次の決定木で再開地点を決定し、`trinity.log` に resumed 行を追記してからループを開始する。

   1. `${RUN_DIR}/eval-*.md` を列挙し、ファイル名から数字を取り出して最大値を `LAST_N` とする（存在しなければ `LAST_N = 0`）
   2. `LAST_N >= 1` の場合: `${RUN_DIR}/eval-${LAST_N}.md` を読んで判定を確認する
      - 本文に `PASS` が含まれ PASS と判定できる場合: ループに入らず、実行手順 4（最終化）に直行する。`trinity.log` への resumed 行は `LAST_N` を使い `=== <RUN_NAME> run resumed from iter <LAST_N> ===` とする。最終出力の `Iters:` は `LAST_N / MAX_ITER` とする
      - 本文に `NEEDS_REVISION` または `FAIL` が含まれ、PASS 以外と判定できる場合: `n = LAST_N + 1` でループに入る。`trinity.log` への resumed 行は `=== <RUN_NAME> run resumed from iter <n> ===` とする
      - いずれにも分類できない（PASS / NEEDS_REVISION / FAIL のいずれの文字列も見つからない）場合は停止し、`eval-${LAST_N}.md` を確認するようユーザーに促す
   3. `LAST_N == 0` かつ `${RUN_DIR}/plan.md` が存在する場合: Planner をスキップし、`n = 1` で Generator チャンク反復から再開する。Evaluator もそのイテレーション内で通常どおり実行する。`trinity.log` への resumed 行は `=== <RUN_NAME> run resumed from iter 1 ===` とする
   4. `LAST_N == 0` かつ `${RUN_DIR}/plan.md` も存在しない場合: 通常起動と同じく `n = 1` で Planner から始める。`trinity.log` への resumed 行は `=== <RUN_NAME> run resumed from iter 1 ===` とする

   **通常ループ**

   - **Planner**: `trinity:planner` サブエージェントを起動。要件（原文ママ）、`Iteration: <n>`、`RUN_DIR`、`WORKTREE_DIR`、`n > 1` なら直前 `eval-<n-1>.md` の存在告知を渡す。resume 経路では `LAST_N == 0` かつ `plan.md` が存在する場合のみ Planner をスキップし既存 `plan.md` をそのまま使う。それ以外（通常起動・`LAST_N >= 1` からのループ再突入・`LAST_N == 0` かつ `plan.md` なしのフォールバック）は通常どおり Planner を起動する。`${RUN_DIR}/plan.md` を書く（再計画時は上書き）。Planner が `AskUserQuestion` を投げた場合はそのまま見せて停止する。
...
6. **ログ**: ループ脱出時または `MAX_ITER` 到達時に `.trinity/trinity.log` に終了行を追記する。
   ...
   resume 経路では、ループ開始前（再開地点決定の直後）に加えて resumed 行を書く。この行は通常起動時の `start` 行（`git-worktree` スキルが書く）と `end` 行（上記）の規約とは別で、resume ごとに 1 行追加される。

   ```shell
   # resume 起動時のみ（ループ開始前に書く）
   printf '=== %s run resumed from iter %d ===\n' "${RUN_NAME}" "$n" >> .trinity/trinity.log
   ```

   `<RUN_NAME>` は `${RUN_DIR}` の basename（`<TS>-<SLUG>`）。`<n>` は再開エントリ時のイテレーション番号（PASS で最終化直行する場合は `LAST_N`、Generator 再開なら `1`、Planner 再開なら `1`、NEEDS_REVISION/FAIL 再突入なら `LAST_N + 1`）。`end` 行のフォーマットは通常経路と同一に保つ。
