...
2. ループの実行：サブエージェントを直列かつ同期的に呼び、作業を実行する。途中から再開する場合は、`RUN_DIR` 内の成果物で到達点を判定する。ループの完了は code-review（サブ手順4）まで含めて初めて成立するため、**`code-review-<n>.md` が存在する最大の `n` を完了済みの最新ループとみなし、その次のループから再開する。** 再開点の判定は以下の3段階で行う。
    ...
    3. Evaluator
    4. code-review: Orchestrator（メイン会話）が子プロセスとして Claude をヘッドレス起動し `/code-review` を実行させる。①スラッシュコマンドはメイン会話でしか実行できない、②`/code-review` は重い機構なのでコンテキスト隔離するため、子プロセス経由とする。レビュー範囲は `<merge-base>..HEAD`（push/upstream 非依存・PR 不要）。
        ...
        非ゼロ終了・空出力・期待形式不一致はレビュー未実施として扱い再試行する。空出力や終了コード0の異常出力を「指摘なし」と誤読してはならない。期待形式は「`### Code review` 見出し＋『Found N issues:』の番号付きリスト」または「`### Code review` 見出し＋『No issues found.』」。再試行は最大3回。3回失敗したらループ番号 `n`・試行回数・失敗理由・`code-review-<n>.md` のパスをユーザーへ報告してエスカレーションし、自動で先へ進まない。
...
3. 次回ループの実行判断：Evaluator の評価・code-review の結果・Generator の停止報告により、以下の表に従って後続対応を判断、実行する。must-fix とは `/code-review` の出力に残った finding すべてを指す（`/code-review` は内部でスコア80未満・ニトピック・pre-existing issue を除外済み）。Orchestrator は `code-review-<n>.md` の `### Code review` セクションに finding が1件以上あれば must-fix あり、「No issues found.」であれば must-fix なしと判定する（各指摘の性質は解釈しない）。完了は Evaluator が PASS かつ must-fix なしの場合のみ。なお code-review は内部で6+エージェントを起動するため評価コストが増える点に留意する。
