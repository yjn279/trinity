...
作成した PR の URL をユーザーへ共有したうえで、マージ確認とクリーンアップ許可を1回の `AskUserQuestion` コール（最大4問）でまとめて確認する。各問は条件付きで提示し、クリーンアップは誤承認を避けるため必ず独立した1問にする。

| 問い | 提示条件 | multiSelect |
| :-- | :-- | :-- |
| マージ候補の選択 | Git Issue が提示されたランのときだけ提示する。選択肢は作成済み PR 群、Other 欄は修正要望の受け口。Git Issue から起票していない場合はこの問いを出さず、PR を作成したまま残してマージはユーザーに委ねる。 | true |
...
| クリーンアップ許可 | 必ず独立した1問として提示する。 | — |

回答は集めた順ではなく、以下の依存順で処理する。

1. **修正チェック（最初）**: マージ問の Other 欄に記入があれば修正要望として該当 Issue の `requirement.md` を更新し手順3から回し直す。この分岐では課題起票もクリーンアップも実行せず、同時に得た課題起票・クリーンアップの回答は破棄する。修正用の独立した選択肢は設けない。修正内容は Other 欄で受ける。
2. **マージ**: 選択された PR を `gh pr merge` でマージする。非選択の PR は据え置く（マージしない）。
3. **課題起票**: 選択された課題を登録する。
