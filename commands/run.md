...
`backlog.tsv` の各行につき、`<RUN_DIR>/pid` の生死と `<RUN_DIR>/status` の有無を組み合わせて起動するかを決める。

| `pid` | `status` | 意味 | 対応 |
| :-- | :-- | :-- | :-- |
| 無い | 無い | 一度も起動していない | 起動する。 |
| ある。プロセスが生存（`kill -0` 成功） | — | 走行中 | 起動しない。手順4の監視に回す。 |
| ある。プロセスが死んでいる（`kill -0` 失敗） | — | 走行中にクラッシュ | 起動する。`loop` が段ごとのチェックポイントから中断点を拾って続きを走る。 |
| 無い | ある（終端: passed／failed／error） | 終端済み | `<RUN_DIR>/redrive` があれば起動する。無ければ手順4・5で処理済みとしてスキップする。 |
