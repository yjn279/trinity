...
- `git-isolated-worktree` — 起動直後、`RUN_DIR` / `WORKTREE_DIR` / `BRANCH` に従って隔離 worktree を作成する
- `git-push-and-pr` — PASS 後の origin への push と PR 作成を一連フローで実行し、`PR_NUMBER` / `PR_URL` を返す
- `git-merge-and-cleanup` — `AskUserQuestion` でマージ可否を確認し、承認時のみ squash マージと worktree/branch/追加パスのお片付けを行う
...
- push に恒久失敗が起きた → 原文を見せて停止（`git-push-and-pr` 参照）
- `git-merge-and-cleanup` のマージ確認 `AskUserQuestion` → ユーザー回答を待つ（最終出力より前）

## 実行手順

1. **準備**: 次の手順で Trinity 固有変数を組み立て、`git-isolated-worktree` を呼ぶ

   **a. 変数の組み立て（アダプタ層 — 本コマンドの責務）**

   ```shell
   BASE_BRANCH=$(git rev-parse --abbrev-ref HEAD)
   TS=$(date -u +%Y%m%dT%H%M%SZ)
   SLUG=<要件から英字 kebab-case 2〜5語、ASCII のみ>
   ...
   slug は「動詞 + 目的語」形が望ましい（例: `add-theme-toggle`）。既存 `.trinity/<TS>-<slug>` と衝突した場合のみ末尾に `-2` `-3` を付ける。

   **b. `git-isolated-worktree` を呼ぶ**

   上で確定した値を次のパラメータとして渡す。
   ...
   スキルが `LOG_PATH` への開始行追記を行う。開始行の形式はスキル側の規約に従う。
...
4. **PASS のとき**: 次を順に実行する
   ...
   **a. PR タイトル / 本文の組み立て（アダプタ層 — 本コマンドの責務）**

   - `PR_TITLE`: `${RUN_DIR}/plan.md` の先頭 H1 本文を取り出す。70 文字超なら冒頭 70 文字で切り詰める。H1 が無ければ要件冒頭 70 文字をフォールバックとして使う
   - `PR_BODY`: 次のテンプレで組み立てる

   ```markdown
   ## 概要
   <plan.md の `## 背景` セクション本文>

   ## ゴール
   <plan.md の `## ゴール` セクション>

   ## 受け入れ基準
   <plan.md の `## 受け入れ基準（Evaluator用チェックリスト）` セクション>
   ...
   **c. `git-merge-and-cleanup` を呼ぶ**

   - `OWNER` / `REPO` = origin URL から取り出す（`git -C "$WORKTREE_DIR" remote get-url origin`）
   - `PR_NUMBER` / `PR_URL` = 上で取得した値
   - `BRANCH` = `$BRANCH`
   - `WORKTREE_PATH` = `$WORKTREE_DIR`
   - `REPO_ROOT` = `$(pwd)`（起動側リポジトリのルート）
   - `EXTRA_CLEANUP_PATHS` = `[$RUN_DIR]`（run ディレクトリ全体を削除する）
   - `CONFIRM_OPTIONS` = `["マージしてお片付け (Recommended)", "PR は残して改善項目を相談する"]`
