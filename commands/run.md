...
作成した PR の URL をユーザーへ共有したうえで、マージ確認とクリーンアップ許可を1回の `AskUserQuestion` コール（最大4問）でまとめて確認する。各問は以下の条件で提示する。
...
1. **修正チェック（最初）**: マージ問の Other 欄に記入があれば、その記入が指す Issue だけを修正要望として `requirement.md` を更新し手順3から回し直す。同じコールで選択された他の Issue の PR は巻き添えにせず、手順2で通常どおりマージする。この分岐が発生した場合、課題起票・クリーンアップは実行せず、同時に得たそれらの回答は破棄する（修正が確定してから改めて実行する）。
