...
- `git-worktree` — 起動直後、要件由来のスラッグを渡して隔離 worktree を作成する。通常起動経路の自動判断結果が `MODE=new-branch` のときは `origin/<デフォルトブランチ>` を base として新規ブランチを切る。`MODE=existing-pr` のときは自動判断で確定した `PR_NUMBER` を渡して PR の head ブランチを worktree にチェックアウトする。TS / SLUG / BRANCH / RUN_DIR / WORKTREE_DIR / BASE_BRANCH / MODE はスキルが返す
- `git-pull-request` — PASS 後の origin への push と PR 作成を一連フローで実行する。`MODE=new-branch` のときは PR タイトルと PR 本文を受けて新規 PR を作成する。`MODE=existing-pr` のときは PR 番号を受けて追加 push のみを行い新規 PR は作成しない。`PR_NUMBER` / `PR_URL` / `PR_REUSED` を返す
- `git-merge` — マージ可否確認の `AskUserQuestion` から、承認時のマージ・後始末、否認時の改善項目ヒアリングまでを内蔵する。呼び出し側から渡すのは PR URL のみ

## 引数

生の引数は `$ARGUMENTS` で受け取る。次の手順で解釈する。順序非依存で、フラグが同時に与えられても処理する（ただし排他ルールは後述）。
...
1. **準備**: `--resume` の有無で次の 2 経路に分岐する。
   ...
   **通常起動（`--resume` なし）**: まず「現在のリポジトリ状態から既存作業を自動判断する」（下記参照）ことで `MODE` と関連変数を確定し、その結果に従って `git-worktree` スキルを呼んで隔離 worktree を作成する。

   呼び出し側で要件文から kebab-case のスラッグ（2〜5 語）を生成する。リポジトリパス・ベースブランチ・ログファイルパスはスキル内で推測する。
   ...
   - `BRANCH` — ブランチ名（`MODE=new-branch` のとき `trinity/<TS>-<SLUG>` 形式。`MODE=existing-pr` のとき PR の head ブランチ名）
   - `RUN_DIR` — run ディレクトリの絶対パス
   - `WORKTREE_DIR` — worktree の絶対パス
   - `BASE_BRANCH` — base ブランチ名
   - `MODE` — 自動判断で確定した `new-branch` または `existing-pr`
   - `PR_NUMBER` — `MODE=existing-pr` のとき。自動判断で確定した PR 番号
   - `PR_URL` — `MODE=existing-pr` のとき。PR の URL

   ### 現在のリポジトリ状態から既存作業を自動判断する

   `--resume` なしで起動した時点で、コマンド側が次の手順でリポジトリ状態を読み取り、`MODE` と関連変数を確定する。

   **判定の入力**（コマンドが起動したリポジトリの状態。ユーザーの cwd を見る）:
   ...
   **判定の決定木**（上から順に評価し、最初にマッチしたものを採用する）:
   ...
   | # | 条件 | 結果 | 動作 |
   | --- | --- | --- | --- |
   | 1 | `CURRENT_BRANCH == DEFAULT_BRANCH` | `MODE=new-branch` | 既存作業判定をスキップし、`git-worktree` スキルを `PR_NUMBER` なしで呼び出す（`origin/<DEFAULT_BRANCH>` 派生の新規ブランチを切る） |
   | 2 | `CURRENT_BRANCH` が `trinity/` プレフィックスを持たない | `MODE=new-branch` | Trinity 起源でないブランチ上での実行は新規ブランチを切る（安全装置） |
   | 3 | `CURRENT_BRANCH` が `trinity/` プレフィックスを持ち、`MATCHED_PR_COUNT == 0` | `MODE=new-branch` | trinity ブランチだが open PR が未作成。新規ブランチを切る |
   | 4 | `CURRENT_BRANCH` が `trinity/` プレフィックスを持ち、`MATCHED_PR_COUNT == 1` | `MODE=existing-pr` | 既存作業として再利用する。`PR_NUMBER = MATCHED_PRS[0].number` に確定し、`git-worktree` スキルに渡す |
   | 5 | `CURRENT_BRANCH` が `trinity/` プレフィックスを持ち、`MATCHED_PR_COUNT >= 2` | 停止 | 曖昧ケース。全 PR 番号と URL をユーザーに表示し「同一ブランチに複数の open PR が紐付いています。意図する PR を確認してから再実行してください」と案内して停止する |

   決定木 #4 で `MATCHED_PRS[0].state` が `OPEN` 以外であれば停止して報告する（理論上ありえないが念のため）。

   **安全装置のまとめ**:
   - 決定木 #1: `CURRENT_BRANCH == DEFAULT_BRANCH`（典型的には `main`）のときは無条件で `MODE=new-branch` を選ぶ。`main` 上での実行が必ず新規ブランチを切る保証となり、意図せず既存 PR を上書きする事故を防ぐ。
   - 決定木 #2: `trinity/` プレフィックスを持たないブランチは、たとえそのブランチに紐づく open PR があっても `MODE=new-branch` を選ぶ。Trinity 起源でないブランチ上の PR への暗黙的な追加 push を防ぐ。
   - 決定木 #5: 同一ブランチに open PR が複数紐付いている曖昧ケースでは停止し、ユーザーに状況を提示して再実行を促す。

   **`git-worktree` スキルの呼び出し**:
   - `MODE=new-branch` の場合: スラッグのみをスキルに渡す。スキルは `origin/<デフォルトブランチ>` を base として新規ブランチを切る。
   - `MODE=existing-pr` の場合: スラッグと `PR_NUMBER` をスキルに渡す。スキルは PR の head ブランチを worktree にチェックアウトする（新規ブランチは作らない）。`MODE=existing-pr` のとき、スラッグは `gh pr view <PR_NUMBER>` で取得した PR タイトルから派生した kebab-case スラッグを呼び出し側で事前に生成する。
...
4. **PASS のとき**: 実行手順 1 で確定した自動判断結果の `MODE` に応じて次を順に実行する。

   **a. PR タイトルと PR 本文を組み立てる（`MODE=new-branch` のときのみ）**

   `MODE=existing-pr` の場合は PR 本文を再構築しない（追加 push のみを行い PR 本体は更新しない）。そのため 4a は `MODE=new-branch` のときだけ実行する。
   ...
   **b. `git-pull-request` を呼ぶ**
   ...
   `MODE` に応じて呼び方が異なる。スキルには `MODE` を渡す。

   **`MODE=new-branch` の場合（新規 PR 作成）**:

   組み立てた PR タイトルと PR 本文、および `MODE=new-branch` をスキルに渡す。worktree パス・ブランチ名・ベースブランチ・リモート情報はスキル内で推測する。

   - `PR_TITLE` = 上で組み立てた PR タイトル
   - `PR_BODY` = 上で組み立てた PR 本文
   - `MODE` = `new-branch`

   返ってきた `PR_NUMBER` / `PR_URL` / `PR_REUSED` を保持する（`PR_REUSED=false`）。

   **`MODE=existing-pr` の場合（既存 PR への追加 push のみ）**:

   PR 本文は渡さない。`MODE=existing-pr` と `PR_NUMBER` をスキルに渡す。スキルは新規 PR を作成せず、既存 PR のブランチへ追加 push するだけである。

   - `MODE` = `existing-pr`
   - `PR_NUMBER` = 実行手順 1 の自動判断で確定した PR 番号

   返ってきた `PR_NUMBER` / `PR_URL` / `PR_REUSED` を保持する（`PR_REUSED=true`）。

   **c. `git-merge` を呼ぶ**

   PR URL だけをスキルに渡す。マージ可否確認の `AskUserQuestion`、承認時のマージ・後始末（リモート/ローカルブランチ削除、worktree 削除、run ディレクトリ削除）、否認時の改善項目ヒアリングまで、すべてスキル内で完結する。ブランチ名・worktree パス・run ディレクトリのパス・リポジトリパスはスキル内で推測する。`MODE=existing-pr` の場合も既存 PR の URL を渡すだけで、スキルの動作は不変である。
   ...
   **d. PASS 後の起票候補ヒアリング（ユーザープロジェクト向け）**
   ...
   PR 作成または追加 push（4b）が成功した経路では、4c の結果（マージ・クローズ・needs-revision）にかかわらず本ステップを実施する。`MODE=existing-pr` でも本ステップは実施する。詳細は後述「PASS後の起票候補ヒアリング（post-run-issue-suggestions）」に従う。`AskUserQuestion` を 1 コール呼び（候補数に応じて 1〜4 問に分割）、選択された候補は即 `gh issue create` する。起票結果は最終出力（ステップ 7）の `Issues:` セクションに反映する。
...
7. **最終出力**: 次のフォーマットでちょうど印字し、2〜3 文の要約を添える。最終化を実施した（PASS で抜けた）場合だけ `PR:` `Merge:` `Cleanup:` の 3 行を加える。`Issues:` セクションは PASS かつ起票が 1 件以上発生したときだけ加える。`Task:` 行は PASS / NEEDS_REVISION / FAIL のいずれの終了経路でも無条件で出す。要件文の表示前に、連続する空白文字（半角・全角スペース、タブ、改行）を半角スペース 1 個に畳み、先頭・末尾の空白を取り除いた上で、200 文字（コードポイント単位）を超える場合は末尾を `…`（U+2026）で切る。

   `PR:` 行は新規作成・既存 PR 追加 push のどちらでも `#<N> <URL>` の同一フォーマットを使う。`Mode:` 行は PASS 時のみ出力し、`new-branch`（新規 PR 作成）または `existing-pr`（既存 PR へ追加 push）を示す。
   ...
   Iters:   <n>/<MAX_ITER>
   Mode:    <new-branch | existing-pr>                               # PASS のときのみ
   PR:      #<PR_NUMBER> <PR_URL>                                   # PASS のときのみ。新規/既存問わず同一フォーマット
   ...
   NEEDS_REVISION / FAIL のまま `MAX_ITER` に到達した場合は `Mode:` `PR:` `Merge:` `Cleanup:` `Issues:` 行を出さない。`Task:` 行は経路にかかわらず常に出す。
