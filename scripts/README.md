...
git の判定は「`git` という語を含まないコマンドは対象外、含むコマンドは判定できる単純な形だけを許す」の1つの規則で行う。単純な形とは、バックスラッシュや展開（`$`・バッククォート）を使わず、引用符を一種類に揃えた、先頭の単一 git コマンド（改行や `&&`・`|`・リダイレクトでつながない）である。作業場所を指す `${WORKTREE_DIR}` だけは値に置き換えてから判定する。この形に収まったコマンドのサブコマンドを許可一覧（deny-by-default）で決め、一覧に無いものと設定の変更（`config`・`-c`。別名の定義やコマンド実行を仕込めるため）は拒否する。判定できない形（引用符やバックスラッシュで `git` の語を組み立てる形を含む）は精密に解析せず、書き直し方を添えて拒否する。引用の中身や、別のコマンドを介した間接実行までは追わず、worktree の隔離と PR レビューを最後の砦とする。拒否理由は子プロセスの Claude が読み、単純な形に直して再実行するため、保守的に拒否しても作業は止まらない。

## テスト

どちらも素の `bash` だけで動く。

- `bash scripts/test-guard.sh` — 権限の許可・拒否を 39 ケースで確かめる。
