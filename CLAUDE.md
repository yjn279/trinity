...
├── agents/              # 各アクターのふるまいとモデル（frontmatter の model: をシェルが読む）
│   ├── planner.md
│   ├── generator.md
│   └── evaluator.md
└── scripts/
    ├── loop.sh          # ループの制御（状態・再開・二重起動の防止）
    ├── steps.sh         # ループの各段（計画・実装・修正・道具・評価）。loop.sh が読み込む
    ├── guard.sh         # アクターの権限を制限するフック。許可・拒否の判断はここが正
