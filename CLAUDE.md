...
最善の実装は、実装しないことである。機能追加には実装コストや保守コストが発生する。ビルドトラップに陥らず本当に必要な機能のみを実装し、まずは実装しない解決策を検討する。

この簡素化の規律は、Trinity が対象プロジェクトに対して行う仕事そのものであり、同時に Trinity 自身（このリポジトリ）にも再帰的に適用する。道具の `/simplify` と Evaluator の「コードの美」「要件妥当性」軸は、まさにこの規律を機械（前段の自動整理）と判断（削れない2割）に分担させたものである。ドキュメントやコードを「もっとシンプルに」と言われたとき、感覚で削り始めない。何を「シンプル」と呼ぶかを先に複数観点で言語化し、確定仕様を物差しに測ってから削る。手順とKPIを以下に定める。

### 簡素化の KPI（多観点）

「シンプルにする」を単一の尺度で測らない。行数だけを見ると、行を削って概念を増やす——表面積は減るが認知負荷は上がる——偽の改善を見逃す。観点を分け、必達・副次・ガードレールを区別する。

| 観点 | 測るもの | 位置づけ |
| :-- | :-- | :-- |
| 認知負荷 | 全体を理解するのに頭に保持すべき概念の数。イベント種別・環境変数・分岐・特例・用語の総量 | 必達 |
| 必要十分性 | すべての要素が確定仕様（`docs/requirements.md`）まで辿れるか。辿れない機能・ノブ・分岐は削除候補 | 必達 |
| 表面積 | 行数・ファイル数・サブコマンド・関数・公開インターフェースの量 | 副次 |
| 結合の単純さ | 部品間の依存と橋渡しの数。特例分岐・暗黙の前提・footgun の少なさ | 副次 |
| 単一の正 | 同じ事実が1箇所だけにあるか。重複・履歴・時系列を持ち込んでいないか | ガードレール |

認知負荷と必要十分性を最優先で下げる。表面積と結合は副次の指標で、概念を増やさずに減らせるなら減らす。表面積を減らすために概念や特例を増やすのは退行である。単一の正は割ってはならない制約として効かせる。

### 簡素化の進め方

1. **KPIを先に、複数観点から言語化する。** 何を「シンプル」と呼ぶかを着手前に定義する。観点を1つに絞ると、表面積だけ削って認知負荷を上げる偽の改善を見抜けない。多観点で現状値（行数・環境変数・イベント種別・用語の数など）を採り、削減の方向を決める。
2. **必要十分性を確定仕様に照らす。** `docs/requirements.md` の確定仕様を物差しに、各要素がそこへ辿れるかを問う。辿れない機能・ノブ・分岐・特例を削除候補に列挙する。
3. **仕様外を削るときは一度ユーザーに確認する。** 「確定仕様にない」は「消すべき」と同義ではない。監視性・コスト調整・再開のように、仕様にはないが運用上有用なアフォーダンスのことがある。削除候補は理由を添えて確認してから削る。
4. **単一の正を守る——重複側を削り、正典は残す。** 同じ事実が2つの文書にあるとき、消すのは正典でない側。Trinity では README が「なぜ」（設計思想）の正典、`commands/run.md` が「手順」の担当である。重複は run.md を削り、README の固有内容は削らない。逆向きに正典を削ると、安く見えて単一の正を割る。
5. **目標は「方向＋ガードレール」で示す。** 「行数を N 行にする」のようなハード数値で縛らない。「概念を減らす・重複を消す・挙動は保つ」の方向と、「確定仕様を割らない・検証を通す」のガードレールで管理する。数値は結果として報告する指標であって、最適化の対象ではない。
6. **挙動を保ったまま削り、削った後に検証する。** このリポジトリでは最低限 `bash -n`・`shellcheck -S warning`。対象プロジェクトでは道具（`/verify`）と Evaluator の PASS が、削減が回帰でないことを担保する。削った部品が解決していた失敗モードが再発しないか、あるいはその失敗モード自体が不要になったかで評価する。

### 簡素化のヒューリスティクス

- **仕様外のノブ・分岐・特例を削る。** 並列上限・タイムアウト・到達しない保留状態のような制御は、まず無くせないかを問う。デフォルト値で固定できるなら環境変数を1つ消せる。
- **判断と配管を分ける。** 「いくつ並列にするか」のような判断はシェルに上限として埋め込まず Orchestrator に委ね、シェルは機械的な起動と監視に徹する。判断をデータ（`backlog.tsv` の行数）に逃がすと、制御コードから分岐が1つ消える。
- **footgun を削る。** ヘッダ行の有無で壊れるパースのような暗黙の前提は、個別に注意書きを足すのではなく、頑健化して特例知識そのものを不要にする。守るべき規約が1つ減る。
