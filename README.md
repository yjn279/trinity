...
| ⑤ | Orchestrator | PASS 時のみ | push 済みブランチ、PR URL と番号 |
| ⑥ | Orchestrator | PR URL・番号 | `AskUserQuestion`（マージ可否確認、1 回目） |
| ⑦ | Orchestrator | 承認時のみ | マージ済み PR、削除済み worktree と branch と `.trinity/<run>/` |
...
│   ├── trinity-branch-push/      # 最終 PASS 後の push と再試行規約・既存ブランチ確認
│   ├── trinity-pr-from-artifacts/  # plan.md / eval-N.md → PR 作成
│   └── trinity-merge-and-cleanup/  # マージ確認・改善項目ヒアリング・squash マージ・お片付け
├── hooks/
