...
### Production-Ready 水準（PASS の最低ライン）

各軸の PASS は、以下の小項目のすべてについて `path:line` 引用付きで満たされていることを意味する。Evaluator は成果物（コード / テスト出力 / 検証チェーン出力）だけで各項目を二値判定する。

#### 機能性

- 計画の `## 受け入れ基準` の機能性項目すべてが、Evaluator が再実行した検証チェーン（型チェック / Lint / ユニット / UI スモーク）で観測可能に PASS している
- 全ユニットテストが PASS（失敗 0。スキップは計画に明記された場合のみ許容）
- 型チェックがエラー 0
- 計画が UI に触れる場合は Playwright スモークが PASS

#### コード品質

- 既存パターンとの整合が確認できる（模倣元が `path:line` で示せる）
- 新規 `any` / `# type: ignore` / `eslint-disable` / `// @ts-ignore` を使用していない。または使用箇所ごとに計画で明示的に許可されている
- デッドコード（未使用 import / 未使用 export / 未参照変数）が存在しない
- Lint がエラー 0。警告は計画で明示的に許容された範囲のみ

#### ビジュアル設計

計画が UI に触れない場合は N/A とし、この軸は PASS 判定の対象外とする。UI に触れる場合は以下をすべて満たす。

- デザイントークン（既存の Tailwind/CSS 変数）から逸脱した新規ハードコード値が存在しない
- 主要操作系のアクセシビリティ最低限を満たす（フォーカス可視・キーボード到達可能・ボタン/リンクに識別可能なラベルが付いている）
- 計画で言及された全ビューポート / テーマで Playwright スモークが PASS

#### 製品としての厚み

- 計画で列挙されたエッジケース（空・エラー・ローディング・境界値）すべてに対応する観測可能な振る舞いがあり、テストか UI スモークで確認できる
- 計画で言及された競合状態（race condition）に対するガード（早期 return、abort signal、idempotency など）が `path:line` で示せる
- エラー経路がサイレント握り潰しになっていない（catch 内 `console.log` のみ等の実装が存在しない）
