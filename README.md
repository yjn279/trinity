...
Anthropic の Planner / Generator / Evaluator パターンを Claude Code のサブエージェント機能で実装したハーネスである。`/trinity:run <要件>` で起動し、隔離された git worktree で実装してコミットし、Evaluator が PASS を返した時点で push と PR 作成を自動で行う。その後 `AskUserQuestion` でマージ可否をユーザーに確認し、承認時はリモートマージとローカルお片付けまでを完結させる。
...
└────────────────────────────────────┬──────────────────────────────────┘
   ...
   └────────────────────────────────────────────────────────────────┘
                ...
                │ AskUserQuestion: マージしますか？   │
                │   1. マージしてお片付け (Recommended)│
                │   2. PR は残して改善項目を相談する   │
                └──────────────────┬──────────────────┘
                         承認時のみ ▼
                ┌─────────────────────────────────────┐
...
```shell
  ...
  ④ ループ n = 1 .. MAX_ITER ────────────────────────────────────┐
        ...
        ▼
  ⑤ 最終化（PASS のときだけ）— git-push-and-pr スキル
        ...
        │
        ▼
  ⑥ マージ確認（AskUserQuestion — 1 回目）— git-merge-and-cleanup スキル
        「PR #N (<URL>) を作成しました。マージしてお片付けまで進めますか？」
        選択肢: 「マージしてお片付け (Recommended)」/ 「PR は残して改善項目を相談する」/ 「Other」
        │
        ├─ 承認 ──▶
  ⑦ マージ＋お片付け（承認時のみ）— git-merge-and-cleanup スキル
        ...
        │
        └─ 否認 ──▶
  ⑦' 改善項目ヒアリング（否認時のみ）— git-merge-and-cleanup スキル
...
| ③ | Orchestrator + `git-isolated-worktree` | `BASE_BRANCH` | `RUN_DIR`, `WORKTREE_DIR`, `BRANCH` |
...
| ⑤ | Orchestrator + `git-push-and-pr` | PASS 時のみ | push 済みブランチ、PR URL と番号 |
| ⑥ | Orchestrator + `git-merge-and-cleanup` | PR URL・番号 | `AskUserQuestion`（マージ可否確認、1 回目） |
| ⑦ | Orchestrator + `git-merge-and-cleanup` | 承認時のみ | マージ済み PR、削除済み worktree と branch と `.trinity/<run>/` |
...
│   ├── git-isolated-worktree/  # 隔離 worktree の作成（branch 名・展開先・base ref を受け取る）
│   ├── git-push-and-pr/        # origin への push と PR 作成を一連フローで実行
│   └── git-merge-and-cleanup/  # マージ確認・改善項目ヒアリング・squash マージ・お片付け
├── hooks/
...
- worktree の後始末はユーザーの承認次第で行う。PASS 後に `AskUserQuestion` でマージを承認した場合は `/trinity:run` が自動でお片付けする（worktree、branch、`.trinity/<run>/` を削除する）。否認した場合はお片付けを行わず、代わりに改善項目のヒアリングを行う。
...
`/trinity:run` を起動した時点で、ユーザーはパイプライン全体（worktree 作成、ブランチ push、PR 作成）への明示的な許可を出したものとして扱う。PR 作成まではユーザー確認を取らずに進める。PR 作成後はマージの可否を `AskUserQuestion` で都度確認し、承認時のみリモートマージとお片付けを行う。NEEDS_REVISION / FAIL のまま `MAX_ITER` に達した場合は、push・PR 作成・マージ確認・お片付けのいずれも行わず、最新の評価レポートのパスを表示して停止する。黙って延々と繰り返さない。
