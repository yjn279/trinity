...
agent 定義は `agents/` に、オーケストレーターの手順は `commands/run.md` に、開発標準プロセスを1サイクル進める自律運転コマンドの手順は `commands/auto.md` に置く。ランの成果物（`plan.md`・`eval-*.md`・`gen-*.md`・`trinity.log` 等）は対象プロジェクトの `.trinity/<run>/` に出力される。worktree は `git-flow` スキルが `.trinity/` の外に切り出す。Pull Request・後片付けといった git 運用も同様に `git-flow` スキルに委譲する。
...
`/trinity:auto` は、Issue の起票からリリースまでの開発標準プロセス（ [`docs/process.md`](docs/process.md) ）を1サイクル進めるコマンドである。`/loop` と組み合わせて起動すると、リリース → 統合 → トリアージ → 実装 → 起票のサイクルが無人で回り続ける。

```shell
/loop /trinity:auto
```

自律運転中のユーザー確認は、プロダクトの軸（ [`docs/product.md`](docs/product.md) ）と開発標準プロセスに基づく既定判断に読み替えられる。軸で判断できない課題は `status: blocked` として人間へ委ねられ、WIP 上限・失敗 Issue の隔離・進捗ゼロでの停止提案といったガードレールが暴走と滞留を防ぐ。プロダクトが成長しても軸がブレないよう、 `docs/product.md` の変更だけは人間の専権として残る。
...
- morphox_ai「Claude Code /loop 解説」 https://aimanavo.com/c/morphox_ai/a/SHjis72jr2Aftw
