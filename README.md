...
**ベースブランチについて**: `/trinity:run`（フラグなし）の既定経路では、現在の作業ブランチに依存せず、常に `origin/<デフォルトブランチ>`（通常 `origin/main`）を base として worktree を作成する。feature ブランチや古い trinity ブランチにいる状態で叩いても、worktree は最新の `origin/main` から派生する。main 以外のブランチにいる場合は、プリフライト hook が stderr に注意書きを表示する（ブロックはしない）。
