...
└────────────────────────────────────┬──────────────────────────────────┘
   ...
   └────────────────────────────────────────────────────────────────┘
                ...
                └──────────────┬──────────────────────┘
                               ...
                               ▼
                ┌─────────────────────────────────────┐
                │ AskUserQuestion（マージ確認）        │
                │   1. Squash and merge + お片付け     │
                │   2. Merge commit + お片付け         │
                │   3. Leave the PR open               │
                └──────────────┬──────────────────────┘
              承認              │              拒否
    ...
    ▼                           │                        ▼
┌─────────────────┐             │          ┌─────────────────────┐
│ merge_pull_     │             │          │ PR をそのまま残す    │
│ request 呼び出し│             │          │ お片付けしない      │
└────────┬────────┘             │          └─────────────────────┘
         ▼                      │
┌─────────────────────────────────────────┐
│ お片付け                                │
...
```shell
  ...
  ⑤ 最終化（PASS のときだけ）
        ...
        ・PR #<番号> と URL をユーザーに表示
        │
        ▼
  ⑤-b マージ確認（PASS のときだけ）
        ・AskUserQuestion でマージ方法をユーザーに確認する（1 回だけ）
            選択肢 1: Squash and merge, then clean up (Recommended)
            選択肢 2: Create a merge commit, then clean up
            選択肢 3: Leave the PR open
        ・承認された場合 → merge_pull_request を呼び出しお片付けへ
        ・拒否された場合 → PR をそのまま残し終了
        │
        ▼
  ⑤-c お片付け（マージ成功時のみ）
        ...
        ・git worktree remove "$WORKTREE_DIR"
        ・git branch -D "$BRANCH"
        ・plan.md と eval-*.md は削除しない（監査ログとして保持）
        │
        ▼
  ⑥ ユーザーへの最終出力
...
| ⑤ | Orchestrator | PASS 時のみ | push 済みブランチ、PR 番号と URL |
| ⑤-b | Orchestrator（AskUserQuestion） | PR 番号・URL | ユーザーの承認または拒否 |
| ⑤-c | Orchestrator | マージ成功時のみ | worktree 撤去、ローカルブランチ削除 |
| ⑥ | Orchestrator | — | 整形された結果サマリ（PR / Merge / Cleanup 行を含む） |
...
- PASS 後にユーザーがマージを承認した場合、ハーネスが自動でお片付けする（worktree ディレクトリの撤去とローカルブランチの削除）。`plan.md` と `eval-*.md` は監査ログとして保持する。ユーザーが `Leave the PR open` を選んだ場合は worktree とブランチをそのまま残し、不要になったら手動で `git worktree remove .trinity/<run>/worktree` して `git branch -D <branch>` する。
...
`/trinity:run` を起動した時点で、ユーザーはパイプライン全体（worktree 作成、ブランチ push、PR 作成）への明示的な許可を出したものとして扱う。push と PR 作成の過程では確認プロンプトは出さない。ただしマージは破壊的な操作のため、PR 作成後に `AskUserQuestion` で別途ユーザー確認を取る（これが唯一の確認ポイントである）。NEEDS_REVISION / FAIL のまま `MAX_ITER` に達した場合は、push と PR 作成は行わず、最新の評価レポートのパスを表示して停止する。黙って延々と繰り返さない。
