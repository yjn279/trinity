...
```shell
  ...
  ⑥ マージ確認（AskUserQuestion）
        ...
        選択肢: 「マージしてお片付け (Recommended)」/ 「PR は残す」/ 「Other」
        │
        ├─ 承認 ──▶
  ⑦ マージ＋お片付け（承認時のみ）
...
| ⑥ | Orchestrator | PR URL・番号 | `AskUserQuestion`（マージ可否確認） |
| ⑦ | Orchestrator | 承認時のみ | マージ済み PR、削除済み worktree と branch |
...
- worktree の後始末はユーザーの承認次第で行う。PASS 後に `AskUserQuestion` でマージを承認した場合は `/trinity:run` が自動でお片付けする。否認した場合は worktree と branch をそのまま残し、ユーザーが手動で `git worktree remove .trinity/<run>/worktree` と `git branch -D <branch>` を実行する。
