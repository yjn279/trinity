...
agent 定義は `agents/` に、オーケストレーターの手順は `commands/run.md` に置く。ランタイム成果物は対象プロジェクトの `.trinity/<run>/` に出力される。worktree の切り出し・Pull Request・後片付けといった git 運用は外部の `git-flow` スキルに委譲する。
