...
┌───────────────────────────────────────────────────────────────────────┐
│ User                                                                   │
│   /trinity:run [--max-iter=N] [--resume=<RUN_DIR>] <1〜4文の要件>          │
└────────────────────────────────────┬──────────────────────────────────┘
                                     ▼
                ┌─────────────────────────────────────┐
...
├── 20260429T153000Z-add-theme-toggle/      # run ディレクトリ（--resume=<このパス> で再開できる）
...
API 課金エラーやレートリミットで途中停止した run を、中間成果物（`plan.md`、`eval-N.md`）から再開できる。

```shell
# RUN_DIR を絶対パスで指定する
/trinity:run --resume=/path/to/.trinity/20260429T153000Z-add-theme-toggle

# リポジトリルートからの相対パスでも指定できる
/trinity:run --resume=.trinity/20260429T153000Z-add-theme-toggle

# --max-iter と組み合わせてイテレーション上限を変更して再開する
/trinity:run --resume=.trinity/20260429T153000Z-add-theme-toggle --max-iter=20
```

`--resume` 時は新規 `RUN_DIR` / worktree / ブランチを作らない。指定した `RUN_DIR` の `worktree/` と既存ブランチをそのまま使う。要件文の再投入は不要である（既存の `plan.md` を再利用する）。既存の `plan.md` を使わず要件から計画し直したい場合は `--resume` を使わず通常起動する。

**再開地点の決定**

| 中間成果物の状態 | 再開地点 |
| --- | --- |
| `eval-N.md` が存在し、最終判定が PASS | 最終化（push + PR）に直行 |
| `eval-N.md` が存在し、最終判定が PASS 以外 | `n = N + 1` でループ再突入（Planner から） |
| `plan.md` のみ存在（`eval-*.md` なし） | `n = 1` で Generator から再開（Planner スキップ） |
| `plan.md` も `eval-*.md` も存在しない | `n = 1` で Planner から（通常起動と同じ） |

**前提条件と制約**

- 同一プロジェクト・同一マシン上での再開のみサポートする。別ホスト・別マシン間の resume は対象外
- 自動再開（クラッシュ検知して自動的に resume）はサポートしない。ユーザーが明示的に `--resume` を渡す
- resume 前に `${RUN_DIR}/worktree` の working tree が clean でなければ停止する（uncommitted な変更を手で commit / stash / discard してから再実行する）
- PASS で最終化に直行する resume 経路でも、post-run のヒアリング（`post-run-issue-suggestions` と `post-run-trinity-self-issue-suggestions`）は通常経路と同様に実施される
...
=== 20260428T091200Z-fix-login-bug run ended: NEEDS_REVISION at iter 1/15 ===
=== 20260428T091200Z-fix-login-bug run resumed from iter 2 ===
2026-04-28T10:02:10Z generator finished on b3c4d5e
2026-04-28T10:05:22Z evaluator finished
=== 20260428T091200Z-fix-login-bug run ended: PASS at iter 2 ===
