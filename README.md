...
ループの離脱には Evaluator の `PASS` と code-review に must-fix（`/code-review` の出力に残った finding）が無いことの両方を要する。条件を満たすとオーケストレーターが push して PR を作成し、1回の `AskUserQuestion` でマージ候補の選択・課題起票・クリーンアップをまとめて確認する。修正要望（Other 欄）があれば手順2へ戻し、確定後に課題起票とクリーンアップを実行する。
...
`/trinity:run` を起動した時点で、worktree 作成・ブランチ push・PR 作成までの許可を出したものとして扱う。PR 確定後は1回の `AskUserQuestion` でマージ候補の選択・課題起票・クリーンアップをまとめて確認する。API 課金エラーやレートリミットで途中停止した場合は、作業環境が残っていれば再実行で続きから再開する。
