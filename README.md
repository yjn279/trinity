...
```shell
  ...
  ④ ループ n = 1 .. MAX_ITER ────────────────────────────────────┐
        ...
        ▼
  ⑤ 最終化（PASS のときだけ）
        ⑤-a git-pull-request スキル
              ・PR タイトル・本文を組み立て（plan.md + eval-<n>.md から）
              ・push -u origin <BRANCH>（リモート同名ブランチを事前チェック）
              ・gh pr create（base = BASE_BRANCH, head = BRANCH）
              → PR URL を返す
        ⑤-b git-merge スキル
              ・AskUserQuestion でマージ方針を確認
              ・マージ / クローズ / Draft 戻しのいずれかを完結
              ・ブランチ削除 + worktree 削除 + run ディレクトリ削除
        │
        ▼
  ⑥ ユーザーへの最終出力
...
| ⑤-a | git-pull-request スキル | PR タイトル、PR 本文 | push 済みブランチ、PR URL |
| ⑤-b | git-merge スキル | PR URL | マージ結果、クリーンアップ完了 |
...
├── skills/
│   ├── git-worktree/
│   │   └── SKILL.md    # 隔離 worktree の作成スキル（入力: スラッグのみ）
│   ├── git-pull-request/
│   │   └── SKILL.md    # push + PR 作成スキル（入力: タイトル・本文のみ）
│   └── git-merge/
│       └── SKILL.md    # PR マージ確認＋クリーンアップスキル（入力: PR URL のみ）
