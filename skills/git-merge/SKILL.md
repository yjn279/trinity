...
when-to-use: "PR のマージ確認とクリーンアップが必要なときに使う。呼び出し側から渡すのは PR URL のみ。通常モード（new-branch）で新規作成した PR でも、既存 PR モード（existing-pr）で追加 push した既存 PR でも、同一の PR URL を渡すだけでよい。"
...
`git-worktree` スキルで `--pr=<N>` 経路を使った場合（`MODE=existing-pr`）でも、PR の head ブランチに紐づく worktree がチェックアウト済みであるため、このスキルの推測ロジックはそのまま動作する。呼び出し側は `MODE` に関わらず PR URL を渡すだけでよい。
...
| worktree パス | `git worktree list --porcelain` からブランチ名で照合 |
