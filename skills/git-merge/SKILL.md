...
when-to-use: "PR のマージ確認とクリーンアップが必要なときに使う。呼び出し側から渡すのは PR URL のみ。新規作成した PR でも、既存 PR を継続して追加 push した PR でも、同一フローで動作する。"
...
PR が新規作成されたものか既存 PR を継続したものかは問わない。どちらの場合も PR の head ブランチに紐づく worktree がチェックアウト済みなので、PR URL から `gh pr view` で `headRefName` を引けば worktree パスを推測できる。
